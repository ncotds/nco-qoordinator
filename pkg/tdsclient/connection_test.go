...
func TestConnection_Exec_Insert(t *testing.T) {
	...
	gotRows, gotAffected, gotErr := conn.Exec(context.Background(), qc.Query{SQL: stmt})
...
func TestConnection_Exec_ContextCancel(t *testing.T) {
	...
	cancel()
	_, _, err := conn.Exec(ctx, qc.Query{SQL: "describe status"})
...
func TestConnection_Exec_Reconnect(t *testing.T) {
	...
	rows, affected, err := conn.Exec(context.Background(), qc.Query{SQL: stmt})
